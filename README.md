...
|**6: IMPORT SIGNED  SERVER SSL CERTIFICATE FOR WEB SERVER:-**<br>We have now Root and SSL Certificate generated in above step. Let’s import & trust the Signed  Server SSL Certificate(.crt) on server:<br>•	Log on to the server <br>•	Open a command prompt (start->run->cmd)<br>•	Change directory to the location of the Java executables (e.g. 'C:\Program Files (x86)\Java\jre6\bin' <br>Use the keytool command again to import & trust /*.sha1.cer certificate: <br>keytool -import -alias [youralias] –file [path/to/.crt file] -keystore [path/to/your/keystore] –trustcacerts <br>Please trust the certificate & verify using below command: <br>keytool -list -v -keystore [path/to/your/keystore] |.crt SSL Certificate should be imported successfully.|
|**7: CONFIGURING YOUR SSL CONNECTOR IN SERVER.XML**<br>•	Please locate the server.xml file from “\Tomcat\conf\server.xml”<br>•	Right click>edit <br>•	If necessary, uncomment the connector. To uncomment a connector, remove the comment tags (<!-- and -->).<br>•	Browse the file to find below parameters:<br><Connector port="8443" maxThreads="150" scheme="https" secure="true" SSLEnabled="true" keystoreFile="path/to/your/keystore" keystorePass="YourKeystorePassword" clientAuth="false" keyAlias="yourAlias" sslProtocol="TLS"/> <br>•	Update the keystoreFile  to path/to/your/keystore and keystorePass to the keystore password provided during 2nd  step.<br>•	Save the server.xml file |Server.xml file should be updated with latest keystore file name & password.|
|**8: RESTART THE TOMCAT SERVER**<br>Please restart service of your Tomcat Server & if you executed every step correctly, Pat yourself  & should be able to access your web application using HTTPS|	https://WEBAPPURL should be accessible.|
